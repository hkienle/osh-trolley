...
# `tn-oshop-assessment` OSH Einkaufstrolley: Bewertung
Stand: November 2025

:::warning
Einstieg in die Dokumentation: https://hackmd.io/@hkienle/osh-trolley-book/%2Fs1F09AVITRW1JFesKfud9A  
...
Ziel der Dokumentation war es verschiedene Ansätzen und Plattformen zu präsentieren und dabei deren jeweilige Stärken auszunutzen. Dadurch ist die Doku allerdings über verschiedene Plattformen verteilt und verlinkt und präsentiert sich dadurch nicht in einem Guss.
...
- **GitLab:** Versionierung von CAD/STL-Dateien, [HackMD-Versionierung](). Feedback zum Projekt via Issues. Interaktiver STL-Viewer.
...
- **Printables:** [CAD/STL-Dateien](https://www.printables.com/@hmk_292473/collections/2752290). Tracking von Mods und Bewertungen anderer Nutzer. Interaktiver STL-Viewer.

![image](https://hackmd.io/_uploads/Sk_XbxFyZg.png =x200)
![image](https://hackmd.io/_uploads/rJSSblt1-l.png =x200)
![image](https://hackmd.io/_uploads/rkvKbxKJ-x.png =x200)  
*2 Rahmenvarianten des Trolleys*
...
Durch die mechanischen Andockpunkte sind (ad-hoc) Mods und Add-Ons relativ einfach möglich. Einzelne Bauteile wie die Räder ermöglichen Varianten in Form von Fertigteilen (COTS) vom Baumarkt/Amazon/AliExpress, 3D-Druck oder Recycling/Upcycling.
...
1. [Abstraktes Design des Rahmens](/@hkienle/frame-design/) (`s`=?mm), unabhängig von konkreten Maßen -- dabei hilft das Gridbeam-System. Der Rahmen hat optionale Add-Ons, auch eine Form von Variabilität.
2. [Design des Rahmens mit Maßen](/@hkienle/frame-17x17/) (z.B.`s`=17mm). Hiermit kann der Rahmen an sich gefertigt werden, allerdings ist noch nicht festgelegt welche Add-Ons und welche Achskonstruktion und Tasche das fertige Produkt haben wird.
3. [Konkreter Bau eines Rahmens](/@hkienle/frame-build-sturdy_cots/), erlaubt die Replizierung eines bereits gebauten Rahmens mit genauer Beschreibung der verwendeten Add-Ons, Design-Instantziierung und Bauteile (BOM).  
  ![image](https://hackmd.io/_uploads/rJZHOgYJbe.png =x200)  
  Dies folgt der Philosopie ["Every build is a fork"](https://www.youtube.com/watch?v=coOF23mOjSc&list=PL6Jpysxw3Ty8tsexf_GvtKM0vFTAel11a&index=5&t=709s) (Marcin J.) im Sinne, dass jeder Nachbau per se eine Variante darstellt (da exakte Replizierung im physischen nie möglich ist).

![image](https://hackmd.io/_uploads/S1xbplYJbg.png =x250)  
*Linker Rahmen mit `s`=17mm und Add-On [COTS-Fahrradgriff](/@hkienle/part-bikehandle_cover), rechter Rahmen mit `s`=13mm und Add-On 3D-Druck-Griff.*
...
- Eine Rahmen-Variante benutzt z.B. Metall-Schrauben die bereits in einer Werkstatt in größerem Umfang vorhanden sind, somit kein extra CO2 durch Verschickung und Verpackung. Da diese Schrauben allerdings Senkkopf haben wurden passende Ringe konstruiert und 3D gedruckt (relativ wenig Material, aber CO2>0).
- Eine Variante des Trolleys verwendet einen bereits vorhandenen Weinkarton als Tasche (Reuse), eine andere Variante verwendet alte Wahlplakate (Upcycling).
- Für eine Rahmen-Variante wurden Kanthölzer mit geringer Profildicke gewählt (13x13mm vs. z.B. 20x20mm) um an Materialvolumen zu sparen und es wurde relativ leichtes und schnellwachsendes Kiefernholz benutzt.
...
![image](https://hackmd.io/_uploads/SkDcagt1-g.png =x250)  
*Eine mögliche hochwertigere Variante durch Kirschholz und Messingschrauben (als Beispiel von einem Gridbeam-Regal)*
...
Die anstehende [oSHOP-Konferenz](https://www.oshop-network.de/konferenz-2025/) soll dazu genutzt werden um mit OSH-Interessierten ins Gespräch zu kommen. Diese haben dann z.B. die Möglichkeit über GitHub Issues und Kommentarfunktionen von HackMD und YouTube öffentlich Input zu geben.
...
Die Tasche hat dabei z.B. zusätlich das Potential als Werbefläche für OSH genutzt zu werden (z.B. auch durch QR Code).  
  ![image](https://hackmd.io/_uploads/BJHolbYyWe.png =x250)  
  *Statt "Mehr Eis" könnte auf der Tasche ein OSH-Slogan stehen, vielleicht etwas (zu?) provizierend: "OSH oder Schlägerei!"*
...
Praxistests und Belastungstests wurden noch gar nicht durchgeführt, diese sind bei einem mechanischen Produkt, welches auch für nicht-technikaffine "Endverbraucher" gedacht ist, aber essentiell. Z.B., durch die ständigen Vibrationen bei der Benutzung können sich Schraubverbindungen über die Zeit lösen.

## Methodik/Vorgehen

Es wurde keiner Endwickungs- und Projektmanagement-Methodik oder ähnliches gefolgt. Im großen und ganzen konnten die Design-Idenen ohne Hindernisse umgesetzt werden. Durch die längeren Entwicklungspausen gab es zwischendurch immer wieder zufällige Ideen, die "hoch gepoppt" sind und Lösungen und neue Varianten ermöglicht haben.
...
Hauptsächliche Treiber, die das Projekt nach vorne gebracht habe waren (leider) Deadlines des Wettbewerbs :neutral_face:. Die größe Herausforderng war Zeitmangel und sich für das Projekt neu zu motiveren und gedankliches wiedereinfinden nach Zwangspausen.

## Referenzen

- [Einstieg in die Dokumentation (HackMD book)](https://hackmd.io/@hkienle/osh-trolley-book/%2Fs1F09AVITRW1JFesKfud9A)
